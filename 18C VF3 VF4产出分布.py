...
vf4=[2.48,2.5,2.55,2.6,2.65,2.68,2.7]
...
for file in glob.glob(os.path.join(path,"*.csv")):
    ...
    # -----------------------各VF4段的颗粒数
    vf4_left=vf4[0]
    for j in range((nvf4s)):
        if j == 0:
            nvf4[0] = nvf4[0] + df.loc[df.VF4 < vf4_left].VF4.count()
        elif j != nvf4s-1:
            # print(j)
            vf4_left =vf4[j-1]
            vf4_right=vf4[j]
            nvf4[j] = nvf4[j] + df.loc[(df.VF4 >= vf4_left) & (df.VF4 <vf4_right)].VF4.count()
        else:
            vf4_left = vf4[j-1]
            nvf4[j] = nvf4[j] + df.loc[df.VF4 >= vf4_left].VF4.count()
...
for i in range(nvf4s):

    if i==0:
        xvf4[i]="<{}".format(vf4[0])
    elif i!=nvf4s-1:
        xvf4[i]="{0}~{1}".format(round(vf4[i-1],2),round(vf4[i],2))

    else :
        xvf4[i]="≥{}".format(vf4[i-1])
...
#VF4分布图
plt.subplot(222)
for a, b in zip(xvf4, nvf4_p):
    b = round(b, 1)
    plt.text(a, b, b, ha='center', va='bottom', fontsize=10)

plt.bar(xvf4, nvf4_p, color='m', label="比例")

plt.title("VF4分布", fontsize=18)
plt.legend(loc="upper left")
plt.xticks(rotation=45, fontsize='small')
