...
while first == True:
    ...
    first = False
    #----------------------------------------请修改需要的20分布分档值
    lopmin, lopmax, lopstep = 180, 240, 20
    vf1min, vf1max, vf1step = 2.95, (3.65+0.0001), 0.1
    vf3min, vf3max, vf3step = (1.5), (2.5+0.0001), 0.1
    irmin, irmax, irstep = 0, (0.9+0.0001), 0.1
    ...
    if paramin != 0:
        ...
        for i in range(num):
            if i != num - 1:
                ...
                x[i] = ">{0}".format(round(paramin + parastep * (i ), 2))
        return x
...
xwld=countx("WLD1",wldmin,wldmax,wldstep)
for file in glob.glob(os.path.join(path, "{0}*.CSV".format(name1))):
    ...
    nwld1 = county('WLD1', wldmin, wldmax, wldstep)
    nwld = [nwld1[i] + nwld[i] for i in range(nwlds)]
...
for a, b in zip(xwld, nwld_p):
    b = round(b, 1)
    plt.text(a, b + 0.1, b, ha='center', va='bottom', fontsize=10)
plt.bar(xwld, nwld_p, color='R', label="比例")
